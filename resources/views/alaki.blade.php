...
$tripMembers = TripMember::where($condition)->get();
...
foreach ($tripPlaces as $tripPlace) {
    ...
    switch ($kindPlaceId) {
        case 1:
            $target = Amaken::whereId($tripPlace->placeId);
            $tripPlace->pic = URL::asset('_images/amaken/' . $target->file . '/' . $target->pic_1);
            $tripPlace->x = $target->C;
            $tripPlace->y = $target->D;
            break;
        case 3:
            $target = Restaurant::whereId($tripPlace->placeId);
            $tripPlace->pic = URL::asset('_images/restaurant/' . $target->file . '/' . $target->pic_1);
            $tripPlace->x = $target->C;
            $tripPlace->y = $target->D;
            break;
        case 4:
            $target = Hotel::whereId($tripPlace->placeId);
            $tripPlace->pic = URL::asset('_images/hotels/' . $target->file . '/' . $target->pic_1);
            $tripPlace->x = $target->C;
            $tripPlace->y = $target->D;
            break;
        case 6:
            $target = Majara::whereId($tripPlace->placeId);
            $tripPlace->pic = URL::asset('_images/hotels/' . $target->file . '/' . $target->pic_1);
            $tripPlace->x = $target->C;
            $tripPlace->y = $target->D;
            break;
...
if($tripNote == null || count($tripNote) == 0)
    $tripNote = "";
else
