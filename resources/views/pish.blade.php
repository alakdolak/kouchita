    ...
    <div class="underMainBox">
        @if(!Auth::check())
            <div class="threeHiddenBox" style="float: left; line-height: 85px">
                <a href="{{route('buyInnerFlight', ['mode' => 1, 'ticketId' => $ticketId, 'adult' => $adult, 'child' => $child, 'infant' => $infant, 'ticketId2' => $ticketId2])}}"> فعلا نه </a>
            </div>
            <div class="threeHiddenBox">
                <div> می خواهم عضو شوم </div>
                <button onclick="document.location.href = '{{route('buyInnerFlight', ['mode' => 2, 'ticketId' => $ticketId, 'adult' => $adult, 'child' => $child, 'infant' => $infant, 'ticketId2' => $ticketId2])}}'" class="btn loginRegister" type="button" style="background-color: #92321b"> ثبت نام کنید </button>
...
<div class="ui_backdrop dark" style="display: none; z-index: 10000000"></div>
    ...
    <script>
        ...
        $('.login-button').click(function() {
            var url = '{{route('preBuyInnerFlight', ['ticketId' => $ticketId, 'adult' => $adult, 'child' => $child, 'infant' => $infant])}}';
            $(".dark").show();
            showLoginPrompt(url);
        });

        function showHalfLogin() {
            var url = '{{route('buyInnerFlight', ['mode' => 3, 'ticketId' => $ticketId, 'adult' => $adult, 'child' => $child, 'infant' => $infant, 'ticketId2' => $ticketId2])}}';
            $(".dark").show();
